...
1. If the integration is unknown, call `crewhelm_search_integrations`. If it is known, skip search.
2. Call `crewhelm_enable_integration` for the chosen integration and retain the returned
   `authConfigId`.
3. Pass that ID directly to `crewhelm_create_connection_link` with a fresh idempotency key.
4. Open the returned short-lived authorization URL yourself. Never send it to an Agent or another
   person.
5. Retain the returned `connectionId`.
6. Call `crewhelm_list_connections` with that exact ID after provider authorization. Exact
   inspection verifies and activates the returned provider account.

## Grant selected tools to an Agent

1. Call `crewhelm_search_integration_tools` for the selected integration.
...
4. Call `crewhelm_configure_agent_connection` with the exact Agent and Connection revisions,
   selected tools, authorization mode, expiry, and the smallest useful duration, output, cost,
   concurrency, and per-Run call limits.
...
- If a write returns an ambiguous reservation, reuse the same request and idempotency key only as
  directed after `recoverAfter`.
- Revoke a Connection with `crewhelm_revoke_authority` to stop local use immediately. Provider-side
  consent and credential deletion remain Composio's responsibility.
