...
CREATE TABLE `__new_agent_schedules` (
	...
	CONSTRAINT "agent_schedules_status" CHECK("__new_agent_schedules"."status" IN ('active', 'paused')),
	...
	CONSTRAINT "agent_schedules_state" CHECK((("__new_agent_schedules"."status" = 'active' AND "__new_agent_schedules"."next_run_at" IS NOT NULL)
        OR ("__new_agent_schedules"."status" = 'paused' AND "__new_agent_schedules"."next_run_at" IS NULL)))
);
--> statement-breakpoint
INSERT INTO `__new_agent_schedules`("schedule_id", "agent_id", "current_revision", "status", "next_run_at", "last_run_id", "last_dispatched_at", "created_at") SELECT 'schedule_' || substr("agent_id", 7), "agent_id", "current_revision", "status", "next_run_at", "last_run_id", "last_dispatched_at", "created_at" FROM `agent_schedules`;--> statement-breakpoint
...
--> statement-breakpoint
INSERT INTO `__new_agent_inbox_items`("item_id", "agent_id", "agent_revision", "fleet_revision", "schedule_id", "schedule_revision", "run_id", "trigger", "run_status", "kind", "approval_count", "request_preview", "result_preview", "reason", "scheduled_at", "retry_at", "occurred_at", "version", "cleanup_at") SELECT "item_id", "agent_id", "agent_revision", "fleet_revision", CASE WHEN "schedule_revision" IS NULL THEN NULL ELSE 'schedule_' || substr("agent_id", 7) END, "schedule_revision", "run_id", "trigger", "run_status", "kind", "approval_count", "request_preview", "result_preview", "reason", "scheduled_at", "retry_at", "occurred_at", "version", "cleanup_at" FROM `agent_inbox_items`;--> statement-breakpoint
...
CREATE TABLE `__new_agent_schedule_revisions` (
	...
	FOREIGN KEY (`agent_id`,`agent_revision`) REFERENCES `agent_revisions`(`agent_id`,`revision`) ON UPDATE no action ON DELETE restrict,
	CONSTRAINT "agent_schedule_revisions_revision_positive" CHECK("__new_agent_schedule_revisions"."revision" > 0),
	CONSTRAINT "agent_schedule_revisions_agent_revision_positive" CHECK("__new_agent_schedule_revisions"."agent_revision" > 0),
	CONSTRAINT "agent_schedule_revisions_name_length" CHECK(length("__new_agent_schedule_revisions"."name") BETWEEN 1 AND 80),
	CONSTRAINT "agent_schedule_revisions_configuration_json" CHECK("__new_agent_schedule_revisions"."configuration" IS NULL OR json_valid("__new_agent_schedule_revisions"."configuration")),
	CONSTRAINT "agent_schedule_revisions_created_at_positive" CHECK("__new_agent_schedule_revisions"."created_at" > 0)
);
--> statement-breakpoint
INSERT INTO `__new_agent_schedule_revisions`("schedule_id", "agent_id", "revision", "agent_revision", "name", "configuration", "created_at") SELECT 'schedule_' || substr("agent_id", 7), "agent_id", "revision", "agent_revision", 'Recurring schedule', "configuration", "created_at" FROM `agent_schedule_revisions`;--> statement-breakpoint
DROP TABLE `agent_schedule_revisions`;--> statement-breakpoint
ALTER TABLE `__new_agent_schedule_revisions` RENAME TO `agent_schedule_revisions`;--> statement-breakpoint
CREATE UNIQUE INDEX `agent_schedule_revisions_schedule_revision` ON `agent_schedule_revisions` (`schedule_id`,`agent_id`,`revision`);
