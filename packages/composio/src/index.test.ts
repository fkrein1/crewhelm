...
describe("Composio catalog adapter", () => {
  ...
  it("lists bounded enabled auth configurations for one exact integration", async () => {
    const apiKey = "composio-project-secret";
    const fetchMock = vi.fn<typeof fetch>().mockResolvedValue(
      catalogResponse({
        items: [
    ...
    const catalog = createComposioCatalog({ apiKey, fetch: fetchMock });
    const result = await catalog.listAuthConfigs({
    ...
    expect(endpoint.origin).toBe("https://backend.composio.dev");
    expect(endpoint.pathname).toBe("/api/v3.1/auth_configs");
    expect(Object.fromEntries(endpoint.searchParams)).toEqual({
      limit: "20",
      show_disabled: "false",
  ...
  it("rejects substituted auth-config toolkit identities", async () => {
    const fetchMock = vi.fn<typeof fetch>().mockResolvedValue(
      catalogResponse({
        items: [
          {
