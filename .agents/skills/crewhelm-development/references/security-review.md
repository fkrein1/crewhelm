...
evidence. Require a second security-focused reviewer only when the change creates or changes a
trust boundary or enforcement mechanism, authentication, authorization, secret lifecycle,
sandboxing or remote execution, destructive behavior, migration recovery, or deployment and
release authority. Existing MCP or provider mutations do not trigger a second reviewer unless
their authority or effect class changes.

Update the threat model when an update trigger applies. Add a decision record only for a durable,
